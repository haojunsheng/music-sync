...
- **354 个单元测试**：全程离线打桩，不触网、不读写用户真实配置
...
`batch` 按**表头名**取值，所以列序随意，但表头名必须是 `title` / `artist`（`album` 可省）：
...
- 编码用 UTF-8（带不带 BOM 都行）
- 表头名写中文（如 `作者,歌曲`）**读不出任何歌曲** —— 名字对不上就是空值，任务会静默跑完 0 首
- 歌名为空的行会被跳过；只有歌名没有歌手时歌手留空
...
.venv/bin/python -m pytest          # 354 个用例
```
